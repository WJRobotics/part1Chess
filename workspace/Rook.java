...
//Hitesh Konduru
//Rook
//Piece moves anywhere into open space horizontally/vertically from its position,if its own color is blocking it it can't go past, if the opposing color is blockingit, the rook can capture it and go onto that square
...
//Precondition: a pieces color is present(a square has a piece with a certaincolor)
...
//postcondition: a list of possible square the piece can properly enter iscreated(horizontally/verically in the same row/column its in, being able to capturepieces of the other color but not its own color)
...
else if (b.getSquareArray()[row][col].getOccupyingPiece().getColor() !=
color){
moves.add(b.getSquareArray()[row][col]);
break;
}
...
else if (b.getSquareArray()[row1][col].getOccupyingPiece().getColor() !=
color){
moves.add(b.getSquareArray()[row1][col]);
break;
}
...
else if (b.getSquareArray()[row2][col1].getOccupyingPiece().getColor() !=
color){
moves.add(b.getSquareArray()[row2][col1]);
break;
}
...
else if (b.getSquareArray()[row2][col2].getOccupyingPiece().getColor() !=
color){
moves.add(b.getSquareArray()[row2][col2]);
break;
}
